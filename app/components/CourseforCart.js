...
const CourseForCart = (props) => {
  ...
  return (
    ...
    <Link id='coursecomdivfc' href={props.href} className={`${props.linkcn}`} >
       <div className={`${props.imagedivcn}`}><Image alt='' src={`${process.env.NEXT_PUBLIC_PORT}/${props.photo.replace(/\\/g, '/')}`} width={300} height={300} quality={50} className={`${props.imagecn}`}/>    </div> 
