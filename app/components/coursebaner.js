...
<Image loading='lazy'	 width={500} height={500} src={props.img} alt={props.alt} className=' absolute sm:right-0 bottom-0  h-96 w-auto' />
    <label className=' sm:bottom-40 rounded-r-3xl md:rounded-none md:rounded-tr-3xl sm:p-5 text-center sm:left-0 sm:justify-start sm:w-1/2 xl:w-2/3 absolute bottom-14 bg-grren-500 bg-opacity-20 w-full flex justify-center  -translate-y-2 text-lg sm:text-xl md:text-2xl  xl:text-3xl  font-black backdrop-blur-lg p-2 text-white'>{props.label}</label>
