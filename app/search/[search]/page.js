...
const Page = async ({searchParams}) => {
  ...
  const fetchI =async() =>{
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_PORT}/api/instructors`, { cache: 'force-cache' });
 ...
 const I = await fetchI()
  ...
  const categoriesKeywordsMap = {     
    ...
    hse: { title: 'السلامة والصحة المهنية',   keywords: ['السلامة في العمل', 'الصحة المهنية', 'الأمان المهني', 'تقييم المخاطر', 'الوقاية من الحوادث', 'إدارة السلامة',
